...
public class PrimeFinder {
	 ...
	 * Both the maximum value and number of threads must be a positive number
	 ...
	 * @throws IllegalArgumentException if start is < 0 or end < start, or if max or
	 *                                  threads are less than 1.
	 */
	public static TreeSet<Integer> findPrimes(int start, int end, int threads) throws IllegalArgumentException {
		positiveRange(start, end);
		...
		for (int i = start; i <= end; i++) {
			...
			queue.execute(new PrimeTask(i, primes));
	...
	private static class PrimeTask implements Runnable {
		...
		public PrimeTask(int number, TreeSet<Integer> primes) {
